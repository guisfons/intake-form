...
const styles = () => {
	...
	const src_admin = utils.srcStylesPath('./wp-admin/wp-admin-styles.scss');
...
gulp.task(
	'dev',
	gulp.series(clean, gulp.parallel(styles, images, fonts), gulp.parallel(scripts, watch, reload))
);

gulp.task('build', gulp.series(clean, fonts, styles, scripts, images, optimize, manifest));
